...
static __INLINE uint8_t wireless_reg_read(uint8_t reg)
{
   uint8_t tx[2]; 
	 uint8_t rx[2];
   tx[0] = NRF24L01_CMD_R_REGISTER | reg;
   tx[1] = 0;
	//uint8_t tx[2] = {NRF24L01_CMD_R_REGISTER | reg, 0}; 
	 wireless_CSN_low();
...
{
   uint8_t addressmode = reg | NRF24L01_CMD_W_REGISTER;
	 uint8_t tx[2];
	 uint8_t rx[2];
	 tx[0] = addressmode;
	 tx[1] = data;
	 //uint8_t tx[2] = {addressmode, data};
	 wireless_CSN_low();
	 spiTx(RF_SPI_BASE, tx, 2, rx);
   wireless_CSN_high();
}
...
static __INLINE void wireless_set_tx_addr(uint8_t  *tx_addr)
{
   uint8_t tx[6];
	 ...
	 tx[0] = NRF24L01_CMD_W_REGISTER | NRF24L01_TX_ADDR_R;
	 tx[1] = tx_addr[0];
	 tx[2] = tx_addr[1];
	 tx[3] = tx_addr[2];
	 tx[4] = tx_addr[3];
...
static __INLINE void wireless_tx_data_payload( uint32_t data)
{
	uint8_t dataArr[5]; 
	uint8_t rx[5];
	dataArr[0] = NRF24L01_CMD_W_TX_PAYLOAD;
	dataArr[1] = data>>24;
	dataArr[2] = data>>16;
	dataArr[3] = data>>8;
	dataArr[4] = data;
  //uint8_t dataArr[5] = {NRF24L01_CMD_W_TX_PAYLOAD, data>>24, data>>16, data>>8, data}; 
	wireless_CSN_low();
	spiTx(RF_SPI_BASE, dataArr, 5, rx);
	wireless_CSN_high();
}
